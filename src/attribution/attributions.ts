...
/** The only licenses mdcovergen is allowed to ship under. */
export const PERMISSIVE_LICENSES = ['MIT', 'MIT AND Zlib', 'OFL-1.1', 'Apache-2.0', 'ISC', '0BSD'] as const;
...
/**
 * The full license text to display for an entry, offline. Composite SPDX
 * expressions such as `MIT AND Zlib` render every license they name.
 */
export function licenseTextFor(attribution: Attribution): string {
  return attribution.license
    .split(' AND ')
    .map((id) => LICENSE_TEXTS[id] ?? '')
    .join('\n\n');
}
