...
- **Three Templates**: Classic (artwork to three edges, type on paper below), Full-bleed (artwork edge to edge, type as an overlay) and Minimal (type only, for mixtapes). Each names its own display, text and Spine faces out of six — five bundled OFL voices and Noto Sans, which is also the universal fallback, CJK included.
